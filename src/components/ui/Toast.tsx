...
export const Toast: React.FC<ToastProps> = ({ message, visible, onClose, type = 'info' }) => {
  ...
  const icon = type === 'error'
    ? (
        <svg className="w-5 h-5 text-red-500 mr-2 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
      )
    : type === 'success'
    ? (
    ...
    : (
        <svg className="w-5 h-5 text-blue-500 mr-2 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><circle cx="12" cy="12" r="10" strokeWidth={2} /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4m0 4h.01" /></svg>
      );
  ...
  return (
    ...
    >
      <div
        className={`flex items-center bg-white border-l-4 ${borderColor} shadow-xl px-6 py-4 rounded-xl text-gray-900 font-medium relative animate-slideDown`}
